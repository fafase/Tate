    ...
    public static IEnumerable TestCasesJumpOver => new[]
{
    ...
    [Test, TestCaseSource(nameof(BishopMovementTest.TestCasesJumpOver))]
    public void Bishop_Jump_Over_Tests(int x, int y, List<(int row, int col)> pawns, List<(int row, int col)> expected)
    {
        SetupWithTestCases(PawnType.Bishop, x, y, pawns);
